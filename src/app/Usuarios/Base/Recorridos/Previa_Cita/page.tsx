...
export default function RecorridosPreviaCita() {
  ...
  useEffect(() => {
    async function cargarDatos() {
      try {
        ...
        // 🔹 Filtrar vehículos en "PREVIA CITA"
        const previaCita = vehiculosArray.filter((v: Vehicle) => {
          const estado = String(v.estado ?? "").trim().toUpperCase();
          return estado.includes("PREVIA") || estado.includes("CITA");
        });

        setVehiculos(previaCita);
      } catch (err) {
      ...
      <main className="min-h-screen bg-black text-white pt-24 px-3 sm:px-8">
        ...
        <div className="flex items-center justify-between mb-8 no-print">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-orange-500 flex items-center gap-3">
            <FaCarSide className="text-orange-400" /> Recorridos — Previa Cita
          </h1>

          <button
            onClick={() => window.open("/api/recorridos_previas", "_blank")}

...
export default function CatalogUsuarios({ data, estado }: Props) {
  // Filtrar por estado
  const base = useMemo(() => {
    return data.filter((v) => (v.estado ?? "").trim().toUpperCase() === "PREVIA_CITA");
  }, [data]);

  // --- Listas únicas ---
  const marcas = useMemo(() => Array.from(new Set(base.map(v => v.marca).filter(Boolean))).sort(), [base]);
  ...
  const transmisiones = useMemo(() => Array.from(new Set(base.map(v => v.transmision).filter(Boolean))).sort(), [base]);
  const tracciones    = useMemo(() => Array.from(new Set(base.map(v => v.traccion).filter(Boolean))).sort(), [base]);
  const carrocerias   = useMemo(() => Array.from(new Set(base.map(v => v.carroseria).filter(Boolean))).sort(), [base]);
  const tapicerias    = useMemo(() => Array.from(new Set(base.map(v => v.tapiceria).filter(Boolean))).sort(), [base]);
  ...
  const modelos = useMemo(() => {
    if (!marca) return Array.from(new Set(base.map(v => v.modelo).filter(Boolean))).sort();
  ...
  }, [base, q, marca, modelo, anioMin, anioMax, precioMin, precioMax, carroseria, transmision, traccion, tapiceria]);
  ...
  const totalPaginas = Math.max(
    1,
    Math.ceil(items.length / VEHICULOS_POR_PAGINA)
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <aside className="md:col-span-1 space-y-3">
          ...
          <div className="p-3 border border-neutral-800 rounded-lg bg-neutral-900">
            ...
            <select className={inputCls} value={marca} onChange={e => { setMarca(e.target.value); setModelo(""); }}>
          ...
          <div className="p-3 border border-neutral-800 rounded-lg bg-neutral-900">
            ...
            <div className="flex gap-2">
              <input type="number" className={inputCls}
                value={anioMin} onChange={e => setAnioMin(Number(e.target.value || minAnioBase))}/>
              <input type="number" className={inputCls}
                value={anioMax} onChange={e => setAnioMax(Number(e.target.value || maxAnioBase))}/>
            </div>
          </div>
          ...
          <div className="p-3 border border-neutral-800 rounded-lg bg-neutral-900">
            ...
            <select className={inputCls} value={carroseria} onChange={e => setCarroseria(e.target.value)}>
              <option value="">Todas</option>
              {carrocerias.map(x => <option key={x} value={x}>{x}</option>)}
            </select>
          </div>
          ...
          <div className="p-3 border border-neutral-800 rounded-lg bg-neutral-900">
            ...
            <select className={inputCls} value={transmision} onChange={e => setTransmision(e.target.value)}>
              <option value="">Todas</option>
              {transmisiones.map(x => <option key={x} value={x}>{x}</option>)}
            </select>
          </div>
          ...
          <div className="p-3 border border-neutral-800 rounded-lg bg-neutral-900">
            ...
            <select className={inputCls} value={traccion} onChange={e => setTraccion(e.target.value)}>
              <option value="">Todas</option>
              {tracciones.map(x => <option key={x} value={x}>{x}</option>)}
            </select>
          </div>
          ...
          <div className="p-3 border border-neutral-800 rounded-lg bg-neutral-900">
            ...
            <select className={inputCls} value={tapiceria} onChange={e => setTapiceria(e.target.value)}>
              <option value="">Todas</option>
              {tapicerias.map(x => <option key={x} value={x}>{x}</option>)}
            </select>
          </div>
          ...
          <button
            onClick={() => {
              setQ(""); setMarca(""); setModelo("");
              setAnioMin(minAnioBase); setAnioMax(maxAnioBase);
              setPrecioMin(""); setPrecioMax("");
              setCarroseria(""); setTransmision(""); setTraccion(""); setTapiceria("");
            }}
        ...
        <section className="md:col-span-3">
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {itemsPaginados.map(v => <VehicleCard key={v.vehiculo_id} v={v} />)}
          </div>
          {items.length === 0 && <p className="text-sm text-neutral-400 mt-4">No se encontraron vehículos.</p>}
        </section>

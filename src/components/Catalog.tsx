...
import { Filter } from "lucide-react"; // Icono de embudo o menú
...
export default function Catalog({ data, estado }: Props) {
  ...
  const inicio = items.length === 0 ? 0 : indiceInicio + 1;
  const fin = Math.min(indiceInicio + itemsPaginados.length, items.length);
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {/* 🔘 Botón Mostrar / Ocultar Filtros (solo móvil) */}
        <button
        ...
        {(mostrarFiltros || isDesktop) && (
          <aside
            className={`
              ...
              md:col-span-1 bg-neutral-900/90 border border-neutral-800 rounded-lg p-4
      ...
      <p className="text-center text-sm text-neutral-400 mt-2">
        Mostrando <span className="text-orange-500">{inicio}</span>–
        <span className="text-orange-500">{fin}</span> de{" "}
        <span className="text-orange-500">{items.length}</span> vehículos
      </p>
    </>
  );
}